...
def create_book(name, image_url, author, genre,   year, owner)
    run_sql("INSERT INTO books(name, image_url, author, genre, year, owner) VALUES($1, $2, $3, $4, $5, $6)", [name, image_url, author, genre, year, owner])
  end
def get_book(id)
    run_sql("SELECT * FROM books WHERE id = $1", [id])[0]
end
...
def book_owner(owner_id)
    run_sql("SELECT * FROM books WHERE owner = $1", [owner_id])
end    
